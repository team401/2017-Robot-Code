...
public class Turret extends Subsystem {
	...
	private boolean isFiring = false, sentryRight = false;

	private int minRPM = 1000, maxRPM = 4600, rpmOffset = 0;
	...
	private void run() {
		...
		if (state.compareTo(TurretState.SENTRY) >= 0) { // auto turret control
			...
			if (Math.abs(turnSpeed) > .5) {
				turretRotator.rotate(turnSpeed > 0 ?
					turnSpeed > .95 ? .10 : .04 :
					turnSpeed < -.95 ? -.10 : -.04);
			} else if (angle != -1) {
				if (angle == 270)
		...
		if (isFiring = (state == TurretState.AUTO && speed != 0) || // auto shooting
				((state == TurretState.SENTRY || state == TurretState.MANUAL) && ControlBoard.INSTANCE.getShootFuel().isTriggered())){ // manual shooting
	...
	public void setWantedState(TurretState state) {
		Robot.getVisionController().setCameraMode(VisionController.Camera.GOAL, state.compareTo(TurretState.SENTRY) >=0 ?
				VisionController.CameraMode.PROCESSING :
				VisionController.CameraMode.STREAMING);
		this.state = state;
	}
